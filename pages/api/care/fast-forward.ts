...
import { fastForwardCare } from '@/src/server/gameplay/careService'
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const orderId = rawOrderId == null ? undefined : Number(rawOrderId)
    if (orderId !== undefined && (!Number.isFinite(orderId) || orderId <= 0)) {
      return res.status(400).json({ error: 'invalid_order_id' })
    }

    const result = await fastForwardCare(session.address, {
      orderId,
    })
    ...
    console.error('[care/fast-forward] failed', err)
    return res.status(500).json({ error: 'care_fast_forward_failed' })
  }
}
